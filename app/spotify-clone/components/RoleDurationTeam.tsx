export default function RoleDurationTeam() {
    return (
        <div className={"grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mt-6 gap-4 justify-between"}>
            <div>
                <h1 className={"font-semibold text-sm"}>Role</h1>
                <p className={"text-sm"}>Full Stack Developer</p>
            </div>
            <div>
                <h1 className={"font-semibold text-sm"}>Duration</h1>
                <p className={"text-sm"}>Apr - July, 2024</p>
            </div>
            <div>
                <h1 className={"font-semibold text-sm"}>Team</h1>
                <p className={"text-sm"}>Just Me!</p>

            </div>
        </div>
    );
}
